...
#### 可选步骤
- **目标曲线补偿**：如果作为数据来源的人工耳，其测量结果与 B&K 4195（入耳式）和 GRAS RA0045+KB0065/66（开放头戴式）+GRAS 45CA（封闭头戴式）有较大出入，建议将 [targets/0_zero.csv/（入耳式）](targets/0_zero.csv/)、[targets/1_zero.csv/（开放头戴式）](targets/1_zero.csv/)、[targets/2_zero.csv/（封闭头戴式）](targets/2_zero.csv/) 替换为数据源的人工耳到上述人工耳的补偿曲线，以兼容用户已经保存的目标曲线。可通过测量多个耳机在官方仓库预设的 Flat Target 下的频响曲线，平均并取反后得到补偿曲线。
